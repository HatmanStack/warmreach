...
export class HttpClient {
            ...
            if (lambdaResponse.statusCode !== 200) {
                const errorBody =
                    typeof lambdaResponse.body === 'string'
                        ...
                        : lambdaResponse.body;
                throw new ApiError({
                    message: (errorBody as Record<string, unknown>)?.error as string || `Lambda returned status ${lambdaResponse.statusCode}`,
    ...
    public async makeRequest<T>(
        ...
        return this.executeWithRetry<T>(
            () => this.apiClient.post<ApiResponse<T>>(endpoint, { operation, ...params }, { signal: options.signal }),
