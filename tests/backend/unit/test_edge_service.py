...
class TestGetPriorityRecommendations:
    """Tests for get_priority_recommendations caching."""

    def _make_service(self, mock_table):
        return EdgeService(table=mock_table)

    def _make_cached_item(self, computed_at=None, recommendations=None, total_eligible=5):
        from datetime import UTC, datetime
        if computed_at is None:
            computed_at = datetime.now(UTC).strftime('%Y-%m-%dT%H:%M:%SZ')
        return {
            'PK': 'USER#test-user',
            'SK': 'INSIGHTS#priority',
            'recommendations': recommendations or [{'profileId': 'cached', 'priorityScore': 80}],
            'totalEligible': total_eligible,
            'computedAt': computed_at,
        }

    def test_cache_hit_returns_cached(self):
        """Cached recommendations within 7 days are returned directly."""
        mock_table = MagicMock()
        cached = self._make_cached_item()
        mock_table.get_item.return_value = {'Item': cached}
        service = self._make_service(mock_table)

        result = service.get_priority_recommendations('test-user')

        assert result['recommendations'][0]['profileId'] == 'cached'
        assert result['totalEligible'] == 5
        # Should not query edges or put_item (no recompute)
        mock_table.query.assert_not_called()
        mock_table.put_item.assert_not_called()

    def test_cache_miss_computes_fresh(self):
        """No cached item triggers fresh computation and caching."""
        mock_table = MagicMock()
        mock_table.get_item.return_value = {}
        mock_table.query.return_value = {'Items': [
            {'PK': 'USER#test-user', 'SK': 'PROFILE#abc', 'status': 'ally', 'messages': []},
        ...
        assert put_item['SK'] == 'INSIGHTS#priority'
    ...
    def test_force_recompute_ignores_cache(self):
        ...
        result = service.get_priority_recommendations('test-user', force_recompute=True)

        # get_item should NOT be called when force_recompute=True
        # (it skips the cache check entirely)
        mock_table.put_item.assert_called_once()

    def test_stale_cache_recomputes(self):
        """Cached item older than 7 days triggers recomputation."""
        mock_table = MagicMock()
        cached = self._make_cached_item(computed_at='2020-01-01T00:00:00Z')
        mock_table.get_item.return_value = {'Item': cached}
        mock_table.query.return_value = {'Items': [], 'LastEvaluatedKey': None}
        mock_table.put_item.return_value = {}
        service = self._make_service(mock_table)
        ...
        # Should recompute and store
        mock_table.put_item.assert_called_once()

    def test_limit_applied_to_cached(self):
        """Requested limit slices cached recommendations."""
        mock_table = MagicMock()
        recs = [{'profileId': f'p{i}', 'priorityScore': 90 - i} for i in range(10)]
        cached = self._make_cached_item(recommendations=recs, total_eligible=10)
        mock_table.get_item.return_value = {'Item': cached}
        service = self._make_service(mock_table)

        result = service.get_priority_recommendations('test-user', limit=3)

        assert len(result['recommendations']) == 3
        assert result['totalEligible'] == 10
