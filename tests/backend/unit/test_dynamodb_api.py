...
class TestNotificationEndpoints:
    """Tests for notification operation handlers in dynamodb-api."""

    def test_get_notifications_returns_200(self, dynamodb_api_module, api_gateway_event_post, lambda_context):
        event = api_gateway_event_post.copy()
        event['body'] = json.dumps({'operation': 'get_notifications'})

        mock_notification_svc = MagicMock()
        mock_notification_svc.get_notifications.return_value = {'notifications': [], 'cursor': None}

        with patch.object(dynamodb_api_module, '_notification_service', mock_notification_svc):
            response = dynamodb_api_module.lambda_handler(event, lambda_context)
    ...
    def test_get_unread_count_returns_200(self, dynamodb_api_module, api_gateway_event_post, lambda_context):
        event = api_gateway_event_post.copy()
        event['body'] = json.dumps({'operation': 'get_unread_count'})

        mock_notification_svc = MagicMock()
        mock_notification_svc.get_unread_count.return_value = {'unreadCount': 3}
    ...
    def test_mark_notification_read_returns_200(self, dynamodb_api_module, api_gateway_event_post, lambda_context):
        event = api_gateway_event_post.copy()
        event['body'] = json.dumps({
            'operation': 'mark_notification_read',
            'notificationSk': 'NOTIFICATION#2026-04-12T00:00:00#abc',
        })

        mock_notification_svc = MagicMock()
        mock_notification_svc.mark_read.return_value = {'success': True}
    ...
    def test_mark_notification_read_missing_sk_returns_400(self, dynamodb_api_module, api_gateway_event_post, lambda_context):
        event = api_gateway_event_post.copy()
        event['body'] = json.dumps({'operation': 'mark_notification_read'})

        response = dynamodb_api_module.lambda_handler(event, lambda_context)

        assert response['statusCode'] == 400
        body = json.loads(response['body'])
        assert 'notificationSk' in body['error']

    def test_mark_all_notifications_read_returns_200(self, dynamodb_api_module, api_gateway_event_post, lambda_context):
        event = api_gateway_event_post.copy()
        event['body'] = json.dumps({'operation': 'mark_all_notifications_read'})

        mock_notification_svc = MagicMock()
        mock_notification_svc.mark_all_read.return_value = {'success': True, 'markedCount': 5}
