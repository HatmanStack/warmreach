...
class BrowserSessionManager {
  static instance: PuppeteerService | null = null;
  static lastActivity: Date | null = null;
  static isAuthenticated = false;
  static sessionStartTime: Date | null = null;
  static errorCount = 0;
  static configManager: typeof ConfigManager = ConfigManager;
  static signalDetector: SignalDetector | null = null;
  static sessionMetrics: SessionMetrics | null = null;
  static contentAnalyzer: ReturnType<typeof getContentAnalyzer> | null = null;
  static backoffController: BackoffController | null = null;
  static _initializingPromise: Promise<PuppeteerService> | null = null;

  /**
   * Get maximum errors from configuration.
   * @returns {number}
   */
  static get maxErrors(): number {
    return this.configManager.getSessionConfig().maxErrors as number;
  }
  ...
  static get sessionTimeout(): number {
    return this.configManager.getSessionConfig().timeout as number;
  }
  ...
  static async getInstance(options = { reinitializeIfUnhealthy: false }) {
    try {
      ...
      if (this.errorCount >= this.maxErrors) {
        await this.cleanup();
        throw new Error(
          `Browser session failed after ${this.maxErrors} attempts: ${(error as Error).message}`
        );
      }
  ...
  static async isSessionHealthy() {
    ...
    try {
      ...
      logger.debug('Session health check failed:', (error as Error).message);
      return false;
    }
  }
  ...
  static async getCurrentUrl() {
    try {
      ...
      logger.debug('Failed to get current URL:', (error as Error).message);
    }
    return null;
  }
   ...
   */
  static setAuthenticationStatus(authenticated: boolean): void {
   ...
   */
  static async recordError(error: Error): Promise<boolean> {
    this.errorCount++;
    logger.warn(
      `Session error recorded (${this.errorCount}/${this.maxErrors}):`,
    ...
    if (this.errorCount >= this.maxErrors) {
      ...
      try {
        ...
        throw new Error(
          `Session recovery failed after ${this.maxErrors} errors: ${(recoveryError as Error).message}`
        );
      }
    }
