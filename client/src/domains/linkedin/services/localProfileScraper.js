...
export class LocalProfileScraper {
  ...
  _extractSkills($) {
    ...
    const items = this._resolveSelector(cheerio.load(section.html()), 'profile:scrape-skill-item');
    if (!items) return [];
   ...
   */
  _extractSubField($el, selectorKey) {
    const cascade = profileSelectors[selectorKey];
    if (!cascade) return '';
  ...
  _extractActivity($) {
    ...
    const activities = [];
    posts.each((i, el) => {
      if (i >= MAX_ACTIVITY_POSTS) return false;
