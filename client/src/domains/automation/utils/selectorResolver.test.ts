...
describe('SelectorResolver', () => {
    const mockRegistry: SelectorRegistry = {
        ...
        'test:params': [
            { strategy: 'aria', selector: 'button[aria-label="{paramName}"]' }
        ]
    };
    ...
    it('resolveRequired throws SelectorNotFoundError on total failure', async () => {
        ...
        await expect(resolver.resolveRequired(page as any, 'test:point'))
    ...
    it('resolveWithWait cascades through waitForSelector with timeouts', async () => {
        ...
        expect(page.waitForSelector).toHaveBeenCalledWith('button[aria-label="test"]', expect.any(Object));
    ...
    it('throws on unknown interaction point', async () => {
        ...
        await expect(resolver.resolve(page as any, 'unknown:point'))
            .rejects.toThrowError(/Unknown interaction point/);
    });
    ...
    it('resolveWithParams replaces placeholders', async () => {
        ...
        const resolver = new SelectorResolver(mockRegistry);
        const result = await resolver.resolveWithParams(page as any, 'test:params', { paramName: 'Save' });
