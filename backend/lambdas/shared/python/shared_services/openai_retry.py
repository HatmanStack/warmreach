...
# backoff starting at RETRY_BACKOFF_BASE_S and doubling each attempt, then
# equal jitter. Keep total wall-clock well under the Lambda timeout (120s): with
# defaults the worst case is 2+4 = 6s of sleep across 3 attempts, and jitter
...
def retry_openai_call(fn, *, max_retries: int = MAX_RETRIES, sleep=None, rng=None):
    """Invoke ``fn`` retrying transient OpenAI errors with jittered backoff.
    ...
    Backoff is **equal jitter**: ``backoff/2 + random(0, backoff/2)``, the same
    shape ``client/src/transport/wsClient.ts`` already uses. Without jitter,
    every concurrent invocation that hit the same 429 slept the identical 2s and
    4s and retried in lockstep, re-colliding with the rate limit it was backing
    off from.

    ``sleep`` resolves to ``time.sleep`` at call time when unset, so tests can
    monkey-patch ``time.sleep`` without re-importing. ``rng`` is a zero-argument
    callable returning a float in ``[0, 1)`` (default :func:`random.random`),
    injectable so the jittered delay is assertable.
    """
    if max_retries < 1:
    ...
    for attempt in range(max_retries):
        try:
            ...
            delay = backoff / 2 + _rng() * (backoff / 2)
            logger.warning(
